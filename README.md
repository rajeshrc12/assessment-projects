https://github.com/user-attachments/assets/175ef601-1bda-49a8-a682-ce388e234bdf
...
- `hooks/` : React hooks for fetching jobs and user data (`useUser.ts`) — they use React Query.
...
- `src/worker/*` : Background worker logic to execute tasks (uses `piscina`/workers).
...
- `piscina` : Worker/pool libraries for task execution.
...
- Frontend data fetching: `client/hooks/useUser.ts` and `client/lib/api.ts`.
